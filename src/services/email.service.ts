...
<body style="margin: 0; padding: 0; background-color: #f1f5f9; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased; -webkit-text-size-adjust: 100%;">
  ...
  /**
   * Retrieves an active template by event key (supporting canonical dot-format and legacy snake_case)
   */
  ...
  public async upsertTemplate(
    ...
    const category = template.category || 'auth_security';
  ...
  public async sendEmail(payload: SendEmailPayload): Promise<boolean> {
    ...
    if (this.db && userId && !bypassPreferences && template.category !== 'auth_security') {
      ...
      if (prefs) {
        if (template.category === 'assessments' && prefs.assessment_reminders === 0) {
          this.logger.info('Email filtered by user preferences', { userId, category: template.category });
          return false;
        }
        if (template.category === 'reports' && prefs.ai_report_alerts === 0) {
          this.logger.info('Email filtered by user preferences', { userId, category: template.category });
          return false;
        }
        if (template.category === 'credits_billing' && prefs.billing_alerts === 0) {
          this.logger.info('Email filtered by user preferences', { userId, category: template.category });
          return false;
        }
        if (template.category === 'system_optional' && prefs.product_updates === 0) {
          this.logger.info('Email filtered by user preferences', { userId, category: template.category });
          return false;
        }
      }
