...
export class AnalyticsService extends BaseService {
  ...
  public async getUserAnalytics(range: AnalyticsRange = '30d') {
    ...
    const [totalUsers, newUsers, verifiedUsers, guestAttempts, assessmentStarts, assessmentCompletions, aiReports, paidSubs] = await Promise.all([
      fetchFirst<{ count: number }>(this.db, 'SELECT COUNT(*) as count FROM users'),
      fetchFirst<{ count: number }>(this.db, 'SELECT COUNT(*) as count FROM users WHERE created_at >= ?', [startDate]),
      fetchFirst<{ count: number }>(this.db, 'SELECT COUNT(*) as count FROM users WHERE email_verified_at IS NOT NULL'),
  ...
  public async getAssessmentAnalytics(range: AnalyticsRange = '30d', assessmentId?: string) {
    ...
    const rows = await executeQuery<any>(
      ...
      `SELECT 
        ...
        (SELECT COUNT(*) FROM ai_generations ai JOIN assessment_attempts att ON ai.attempt_id = att.id WHERE att.assessment_id = a.id AND ai.status = 'completed' AND ai.created_at >= ?) as ai_reports,
        (SELECT AVG((julianday(att.completed_at) - julianday(att.started_at)) * 86400) FROM assessment_attempts att WHERE att.assessment_id = a.id AND att.status = 'completed' AND att.created_at >= ?) as avg_duration
       FROM assessments a
       LEFT JOIN assessment_categories c ON a.category_id = c.id
       ${whereClause}
       ORDER BY completions DESC`,
      [startDate, startDate, startDate, startDate, ...whereParams]
    );
    ...
    const items: AssessmentAnalyticsItem[] = rows.map((r) => {
      ...
      return {
        ...
        avgDurationMinutes: r.avg_duration ? Math.max(1, Math.round(r.avg_duration / 60)) : 3
      };
    });
  ...
  public async getAiAnalytics(range: AnalyticsRange = '30d'): Promise<AiAnalyticsSummary> {
    ...
    const [aggregateRow, providerRows, assessmentRows] = await Promise.all([
      ...
      executeQuery<{ assessment_name: string; reports: number; tokens: number; cost: number }>(
        ...
        `SELECT 
         ...
         LEFT JOIN assessment_attempts att ON ai.attempt_id = att.id
         LEFT JOIN assessments a ON att.assessment_id = a.id
         WHERE ai.created_at >= ?
         GROUP BY a.id
         ORDER BY cost DESC`,
        [startDate]
    ...
    return {
      ...
      successRate,
      promptTokens: aggregateRow?.input_tokens || 0,
      completionTokens: aggregateRow?.output_tokens || 0,
  ...
  public async getSystemHealth(range: AnalyticsRange = '30d'): Promise<SystemHealthSummary> {
    ...
    const [emailStats, failedAi, failedWebhooks, recentAuditErrors] = await Promise.all([
      ...
      executeQuery<{ action: string; entity_type: string; details: string; created_at: string }>(
        this.db,
        `SELECT action, entity_type, details, created_at FROM audit_logs
    ...
    return {
      ...
      recentErrors: recentAuditErrors.map((a) => ({
        ...
        action: a.action,
        error: a.details ? a.details.substring(0, 100) : 'System notice',
