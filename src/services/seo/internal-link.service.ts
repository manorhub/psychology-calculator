...
export class InternalLinkService extends BaseService {
  ...
  public async getRelatedAssessments(
    ...
    >(
      this.db,
      `SELECT r.*, a.title as target_title, a.slug as target_slug, a.description as target_desc, c.name as cat_name
    ...
    if (results.length < limit && categoryId) {
      ...
      >(
        ...
        `SELECT a.*, c.name as cat_name
         ...
         ORDER BY a.popularity_score DESC, a.created_at DESC LIMIT ?`,
      ...
      for (const asm of categoryAssessments) {
        if (!seenIds.has(asm.id) && results.length < limit) {
          ...
          results.push({
            ...
            anchorText: `Explore ${asm.title}`,
            description: asm.description || undefined,
  ...
  public async getRelatedCategories(currentCategoryId?: string | null, limit = 5): Promise<RelatedLink[]> {
    ...
    const query = currentCategoryId
      ? `SELECT * FROM assessment_categories WHERE is_active = 1 AND id != ? ORDER BY display_order ASC LIMIT ?`
      : `SELECT * FROM assessment_categories WHERE is_active = 1 ORDER BY display_order ASC LIMIT ?`;
