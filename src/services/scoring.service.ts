...
export class ScoringService extends BaseService {
  ...
  public async calculateAttemptScores(attemptId: string): Promise<ScoringCalculationResult> {
    ...
    // Calculate maximum possible scores per dimension to compute normalized percentage
    for (const rule of scoringRules) {
      const dimEntry = dimensionMap.get(rule.dimension_id);
      if (dimEntry) {
        // Group max rule score per question
        dimEntry.maxPossible += Math.max(rule.score, 1) * (rule.weight || 1.0);
      }
    }
    ...
    for (const [, entry] of dimensionMap) {
      ...
      // Find matching result archetype for this dimension
      const dimResultType = resultTypes.find(
        (rt) =>
          rt.dimension_id === entry.dim.id &&
          entry.rawScore >= rt.minimum_score &&
          entry.rawScore <= rt.maximum_score
      );
    ...
    let primaryResult = resultTypes.find(
      (rt) => !rt.dimension_id && totalRaw >= rt.minimum_score && totalRaw <= rt.maximum_score
    );
