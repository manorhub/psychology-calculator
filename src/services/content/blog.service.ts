...
export class BlogService extends BaseService {
  ...
  public async getPublishedPosts(options: PostFilterOptions = {}): Promise<PostListResult> {
    ...
    const conditions: string[] = [
      "p.status = 'published'",
      "(p.published_at IS NULL OR datetime(p.published_at) <= datetime('now'))"
    ];
    const params: any[] = [];
  ...
  public async getPostBySlug(slug: string, allowUnpublished = false): Promise<PostWithRelations | null> {
    ...
    if (!allowUnpublished) {
      sql += " AND p.status = 'published' AND (p.published_at IS NULL OR datetime(p.published_at) <= datetime('now'))";
    }
  ...
  public async getFeaturedPost(): Promise<PostWithRelations | null> {
    ...
    const sql = `
      ...
      LEFT JOIN authors a ON p.author_id = a.id
      LEFT JOIN blog_categories c ON p.category_id = c.id
      WHERE p.status = 'published' AND (p.published_at IS NULL OR datetime(p.published_at) <= datetime('now')) AND p.featured = 1
  ...
  public async getRelatedPosts(currentPostId: string, categoryId?: string | null, limit = 3): Promise<PostWithRelations[]> {
    ...
    let query = `
      ...
      FROM posts p
      LEFT JOIN blog_categories c ON p.category_id = c.id
      WHERE p.status = 'published' AND (p.published_at IS NULL OR datetime(p.published_at) <= datetime('now')) AND p.id != ?
    `;
  ...
  public async upsertPost(input: UpsertPostInput): Promise<string> {
    ...
    let isNew = true;
    if (input.id) {
      const existingRecord = await fetchFirst<{ id: string }>(
        this.db,
    ...
    await executeMutation(
      ...
      `INSERT INTO post_versions (id, post_id, version_number, title, content, excerpt)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [crypto.randomUUID(), id, nextVer, input.title.trim(), input.content, input.excerpt?.trim() || null]
    );
