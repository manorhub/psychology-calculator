...
import { ValidationError, NotFoundError, ExternalServiceError, BadRequestError } from '@/lib/errors';
...
export class AssessmentTranslationService extends BaseService {
  ...
  public async generateAiTranslation(
    ...
    if (targetLocale === 'en') {
      throw new BadRequestError('English is the source language and cannot be selected as a translation target.');
    }

    if (!isValidLocale(targetLocale) || !['es', 'fr', 'de', 'pt', 'hi'].includes(targetLocale)) {
      throw new BadRequestError(`Unsupported target language: ${targetLocale}. Supported target languages: es, fr, de, pt, hi.`);
    }
...
6. OUTPUT STRICTLY VALID JSON matching the specified JSON schema. Do not include markdown code fences or conversational text.`;
  ...
  public async saveTranslation(
    ...
    if (targetLocale === 'en') {
      throw new BadRequestError('English is the source language and cannot be saved as a translation.');
    }

    if (!isValidLocale(targetLocale)) {
      throw new BadRequestError(`Invalid target locale: ${targetLocale}`);
    }
