  ...
  public async getActiveInProgressAttempt(userId: string): Promise<UserAssessmentItem | null> {
    ...
    const row = await fetchFirst<{
      ...
      `SELECT
       ...
       JOIN assessment_categories c ON asm.category_id = c.id
  ...
  public async getUserResults(userId: string, limit = 50): Promise<UserResultItem[]> {
    ...
    const rows = await executeQuery<any>(
      ...
      `SELECT
         ...
         asm.slug as assessment_slug,
         c.name as category_name,
         COALESCE((CAST((strftime('%s', a.completed_at) - strftime('%s', a.started_at)) AS INTEGER)), 0) as duration_seconds,
         a.completed_at,
         rs.snapshot_data
       FROM assessment_attempts a
       JOIN assessments asm ON a.assessment_id = asm.id
    ...
    const resultService = new ResultService(this.db);
    ...
    return Promise.all(
      rows.map(async (r) => {
        ...
        if (!snapshotData) {
          try {
            snapshotData = await resultService.createOrGetSnapshot(r.attempt_id);
          } catch {}
        }
        ...
        return {
          ...
          durationSeconds: snapshotData?.durationSeconds || r.duration_seconds || 0,
