...
export class AssessmentCategoryService {
  ...
  async getCategoryBySlug(slug: string, includeNonActive = false): Promise<AssessmentCategoryRow | null> {
    ...
    let query = `
      ...
      WHERE c.slug = ?
    `;
    const params: any[] = [slug.trim().toLowerCase()];
  ...
  async createCategory(input: CreateCategoryInput, actorId = 'system'): Promise<AssessmentCategoryRow> {
    ...
    const shortDesc = input.short_description ? sanitizeHtml(input.short_description.trim()) : null;
    const desc = input.description ? sanitizeHtml(input.description.trim()) : null;
    ...
    await this.auditService.log({
      ...
      entity: 'category',
      entityId: id,
      metadata: { name, slug, status, featured, displayOrder }
    });
  ...
  async updateCategory(id: string, input: UpdateCategoryInput, actorId = 'system'): Promise<AssessmentCategoryRow> {
    ...
    const shortDesc = input.short_description !== undefined
      ? (input.short_description ? sanitizeHtml(input.short_description.trim()) : null)
      : current.short_description;
    const desc = input.description !== undefined
      ? (input.description ? sanitizeHtml(input.description.trim()) : null)
      : current.description;
