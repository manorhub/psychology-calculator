...
export class DeepSeekProvider implements AIProvider {
  ...
  public async generateStructured(
    ...
    if (!apiKey) {
      throw new ExternalServiceError('DEEPSEEK_API_KEY is not configured in server environment');
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ExternalServiceError(`DeepSeek API error (${response.status}): ${errorText}`);
      }
  ...
  public async testConnection(apiKey?: string, model = 'deepseek-chat'): Promise<{ success: boolean; latencyMs: number; error?: string }> {
