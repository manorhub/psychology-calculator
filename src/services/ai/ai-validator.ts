...
export const AIFinalSynthesisSchema = z.object({
  top_takeaways: z.array(z.string()).default([]),
  strongest_pattern: z.string().default(''),
  biggest_growth_opportunity: z.string().default(''),
...
export class AIValidator {
  /**
   * Resiliently cleans and repairs raw JSON strings from LLMs
   */
  public static cleanAndRepairJson(raw: string): any {
    let text = (raw || '').trim();

    // 1. Strip markdown codeblocks e.g. ```json ... ```
    if (text.startsWith('```')) {
      text = text.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, '').trim();
    }

    // 2. Extract JSON object substring if model added preamble/postamble text
    ...
    // 3. Fast direct attempt
    try {
      return JSON.parse(text);
    } catch {
      // Continue to sanitization repairs
    }
    ...
    // 6. Try parsing after comments & trailing commas removed
    try {
      return JSON.parse(text);
    } catch {
      // Continue to structural & truncation repair
    }

    // 7. Structural character repair & unclosed bracket completion
    text = AIValidator.repairTruncatedJson(text);

    try {
      return JSON.parse(text);
    } catch (finalErr: any) {
      throw new ValidationError(`Failed to parse AI response as JSON: ${finalErr.message}`);
    }
  }

  /**
   * Repairs truncated strings, unescaped newlines, and unclosed JSON objects/arrays
   */
  ...
  public static validateAndSanitize(rawJson: string): AIReportContent {
    const parsed = AIValidator.cleanAndRepairJson(rawJson);
    ...
    return {
      ...
      final_synthesis: data.final_synthesis
        ? {
            top_takeaways: data.final_synthesis.top_takeaways?.map(sanitizeStr).filter(Boolean) || [],
            strongest_pattern: sanitizeStr(data.final_synthesis.strongest_pattern),
            biggest_growth_opportunity: sanitizeStr(data.final_synthesis.biggest_growth_opportunity),
