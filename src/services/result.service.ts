...
export class ResultService extends BaseService {
  ...
  public async getResult(
    ...
    // 3. Dynamic Guest Controls Check
    if (!userId && !attempt.user_id) {
      const resultsEnabled = await fetchFirst<{ value: string }>(
        this.db,
        "SELECT value FROM site_settings WHERE key = 'guest_results_enabled'"
      );
      if (resultsEnabled && resultsEnabled.value === 'false') {
        throw new ForbiddenError('Guest results viewing is currently disabled. Please sign in to view results.');
      }

      // Check retention window
      const retentionSetting = await fetchFirst<{ value: string }>(
        this.db,
        "SELECT value FROM site_settings WHERE key = 'guest_result_retention_days'"
      );
      const retentionDays = retentionSetting ? parseInt(retentionSetting.value, 10) : 7;
      if (retentionDays > 0 && attempt.completed_at) {
        const completedDate = new Date(attempt.completed_at).getTime();
        const expirationDate = completedDate + retentionDays * 24 * 60 * 60 * 1000;
        if (Date.now() > expirationDate) {
          throw new ForbiddenError('This guest result has expired. Please create an account on your next assessment to save your results permanently.');
        }
      }
    }
  ...
  public async getRelatedAssessments(
    ...
    limit = 3
  ): Promise<AssessmentWithCategory[]> {
    if (!this.db) return [];

    return executeQuery<AssessmentWithCategory>(
      this.db,
