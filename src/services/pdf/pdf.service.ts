...
export class PdfService extends BaseService {
  ...
  public async generateResultPdf(
    ...
    const builder = new PdfDocumentBuilder({
      brandName: siteConfig.settings.pdf_brand_name || siteConfig.siteName || 'Psychology Calculator',
      brandDomain: siteConfig.settings.pdf_brand_domain || 'psychologycalculator.com',
      primaryColor: siteConfig.settings.pdf_primary_color || '#4f46e5',
      secondaryColor: siteConfig.settings.pdf_secondary_color || '#0ea5e9',
      footerText: siteConfig.settings.pdf_footer_text || 'Psychology Calculator — Official Psychometric Evaluation Report',
      disclaimerText: snapshot.disclaimer || siteConfig.settings.pdf_disclaimer
    });
    ...
    // Header & Title
    builder.addHeader(snapshot.categoryName, snapshot.assessmentName);
    builder.addTitleBlock(snapshot.assessmentName, snapshot.categoryName, displayName, reportDate);
    ...
    builder.addOutcomeCard(
      ...
      snapshot.primaryResultType?.summary || 'Standardized psychometric outcome calculated from verified item responses.'
    );
    ...
    if (snapshot.sections && snapshot.sections.length > 0) {
      ...
      const secData: SectionContentData[] = snapshot.sections.map((s) => ({
        title: s.title,
        content: s.content
      }));
    ...
    // Disclaimer
    builder.addDisclaimer(snapshot.disclaimer);
  ...
  public async generateAiReportPdf(
    ...
    if (report.content) {
      try {
        aiContent = JSON.parse(report.content);
      } catch {
        aiContent = { executive_summary: report.content };
      }
    }
    ...
    builder.addOutcomeCard(
      ...
      snapshot.totalNormalizedScore || 0,
      snapshot.primaryResultType?.summary || 'Standardized baseline assessment outcome.'
    );
    ...
    if (existing) {
      ...
      if (obj && obj.data) {
        ...
        if (obj.data instanceof Uint8Array) {
          ...
          return this.generateResultPdf(attemptId, { userDisplayName: user?.profile?.displayName });
        }
  ...
  public async getOrGenerateAiReportPdf(
    ...
    if (!user || (user.id !== report.user_id && user.role !== 'admin' && user.role !== 'super_admin')) {
    ...
    if (existing) {
      ...
      if (obj && obj.data) {
        ...
        if (obj.data instanceof Uint8Array) {
          ...
          return this.generateAiReportPdf(reportId, { userDisplayName: user?.profile?.displayName });
        }
        return { fileRecord: existing, pdfBytes };
      }
    }
