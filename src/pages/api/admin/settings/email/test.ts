      ...
      ? EmailService.createFromSmtpConfig(
          {
            ...
            fromName: settings.smtp_from_name || 'Psychology Calculator',
            fromEmail: settings.smtp_from_email || 'noreply@psychologycalculator.com',
            replyTo: settings.smtp_reply_to
          },
          settings.site_name || 'Psychology Calculator'
        )
      : new EmailService(db, undefined, settings.site_name || 'Psychology Calculator');

    const result = await emailService.sendTestEmail(data.recipient);
