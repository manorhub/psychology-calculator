...
const questionSchema = z.object({
  ...
  question_text: z.string().min(2, 'Question text is required'),
  question_type: z.enum(['likert', 'multiple_choice', 'yes_no', 'ranking']).default('likert'),
...
export const POST: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    const data = validateSchema(questionSchema, body);

    const env = locals.runtime?.env;
    const db = getD1Database(env);
    const service = new AssessmentBuilderService(db);

    const saved = await service.saveQuestion(id, data, adminUser.id);
