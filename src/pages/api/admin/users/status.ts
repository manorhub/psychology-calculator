...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const data = validateSchema(statusSchema, body);

    const env = locals.runtime?.env;
    const db = getD1Database(env);
    const adminService = new AdminService(db);

    await adminService.updateUserStatus(data.userId, data.status, adminUser.id);

    const response: ApiResponse = {
      success: true,
      data: { message: `User status updated to ${data.status}` },
