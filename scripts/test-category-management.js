...
async function runTests() {
  ...
  assert(searchResults.some((c) => c.slug.includes('mindfulness') || c.name.includes('Mindfulness') || (c.description && c.description.includes('mindfulness'))), 'Search result should match query');
  ...
  const demoJson = importExportService.generateDemoTemplate();
  demoJson.assessment.category_slug = 'personality'; // valid category
  const validResult = await importExportService.validateJson(demoJson);
  ...
  const invalidResult = await importExportService.validateJson(demoJson);
  assert(!invalidResult.valid, 'JSON Import must reject non-existent category slug');
  assert(invalidResult.errors.some((e) => e.field === 'category_slug' || e.message.includes('category')), 'Error should specify category');
  ...
  const badCategoryCsv = demoCsv.replace(/mental-wellbeing/g, 'non-existent-random-category-1234');
  ...
  const auditLogs = sqlite.prepare("SELECT action, entity_type, entity_id FROM audit_logs WHERE entity_type = 'category'").all();
