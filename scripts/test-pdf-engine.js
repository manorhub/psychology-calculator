...
async function runPdfEngineTests() {
  ...
  rawDb.prepare("INSERT INTO user_profiles (user_id, display_name) VALUES (?, 'Alice Johnson')").run(userIdA);

  rawDb.prepare("INSERT INTO users (id, email, role, status) VALUES (?, ?, 'user', 'active')").run(
    userIdB,
    'bob@example.com'
  );
  rawDb.prepare("INSERT INTO user_profiles (user_id, display_name) VALUES (?, 'Bob Smith')").run(userIdB);

  await creditService.grantBonusCredits(userIdA, 25, 'Test bonus');

  // Complete Big Five Assessment for User A
  const bigFive = await runtimeService.getPublicAssessment('big-five-personality-test');
  assert.ok(bigFive);

  const attemptA = await runtimeService.startAttempt(bigFive.id, userIdA, 'sess_alice');
  ...
  const aiReport = await aiService.generateReport(attemptA.id, userIdA);
  assert.ok(aiReport);

  const { fileRecord: aiFile, pdfBytes: aiBytes } = await pdfService.generateAiReportPdf(aiReport.id, {
    userDisplayName: 'Alice Johnson'
  });
  ...
  // Check R2 storage
  const storedAi = await mockR2Bucket.get(`reports/ai/${aiReport.id}.pdf`);
  assert.ok(storedAi);
  assert.strictEqual(storedAi.key, `reports/ai/${aiReport.id}.pdf`);
  ...
  const cachedAi = await pdfService.getOrGenerateAiReportPdf(aiReport.id, userAObj);
  assert.strictEqual(cachedAi.fileRecord.r2_key, `reports/ai/${aiReport.id}.pdf`);
  ...
  await assert.rejects(
    async () => {
      await pdfService.getOrGenerateAiReportPdf(aiReport.id, userBObj);
    },
  ...
  const guestAttempt = await runtimeService.startAttempt(bigFive.id, null, 'sess_guest_999');
  for (const q of bigFive.questions) {
  ...
  await configService.updateSettings(
    {
      pdf_brand_name: 'Custom Psychology Lab',
      pdf_brand_domain: 'custompsychologylab.com',
      pdf_disclaimer: 'Custom scientific psychometric research disclaimer.'
    },
    'admin_1'
  );

  const updatedConfig = await configService.getSiteConfig();
  assert.strictEqual(updatedConfig.settings.pdf_brand_name, 'Custom Psychology Lab');
  assert.strictEqual(updatedConfig.settings.pdf_brand_domain, 'custompsychologylab.com');
