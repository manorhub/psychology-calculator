...
async function runQuestionExperienceTests() {
  ...
  // Verify One Question at a time architecture & Dimension Context
  ...
  assert.ok(takeAstroContent.includes('data-dim-id'), 'Must track dimension IDs on slides');
  assert.ok(takeAstroContent.includes('data-dim-name'), 'Must track dimension names on slides');
  assert.ok(takeAstroContent.includes('What We\'re Exploring'), 'Must display What We\'re Exploring label');
  assert.ok(takeAstroContent.includes('dimension-transition-card'), 'Must provide ✦ NEW AREA dimension transition card');
  ...
  console.log('✔ Component structure, dynamic dimension context, and accessibility markers verified');
  ...
  const attemptData = await runtimeService.getAttemptProgress(attempt.id, testUserId, null);
  ...
  assert.ok(completionResult.dimensionScores.length > 0, 'Scoring engine must compute dimension scores');
  assert.ok(typeof completionResult.totalNormalizedScore === 'number', 'Total normalized score must be computed');
  console.log(`✔ Attempt finalized with ${completionResult.dimensionScores.length} dimensional scores and normalized score: ${completionResult.totalNormalizedScore}%`);
