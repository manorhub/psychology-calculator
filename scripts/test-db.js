...
async function runTests() {
  ...
  assert.ok(categories.length >= 4, 'Expected at least 4 seeded categories');
  ...
  assert.ok(assessments.length >= 1, 'Expected at least 1 featured assessment');
  assert.ok(assessments[0].slug.startsWith('big-five-personality'));
  ...
  assert.ok(resultTypes.length >= 3, 'Expected at least 3 result types');
  assert.ok(resultTypes[0].contents.length >= 3, 'Expected at least 3 content sections for High Openness');
  ...
  assert.ok(aiConfigs.length >= 2, 'Expected at least 2 AI configs');
  ...
  assert.ok(dynamicConfig.siteName === 'Psychology Calculator' || dynamicConfig.siteName === 'MindMetrics', 'Site name should match configured brand');
